...
namespace Pinwheel.Griffin
{
    public class GGeometry : ScriptableObject
    {
        ...
        [SerializeField]
        private int heightmapVersion;
        private const int HEIGHT_MAP_VERSION_ENCODE_RG = 246;
        ...
        {
            int count = 0;
            List<string> keys = TerrainData.GeometryData.GetKeys();
            for (int i = 0; i < keys.Count; ++i)
            {
                bool delete = false;
                if (!keys[i].StartsWith("~Chunk"))
                {
                    delete = true;
                }
                else
                {
                    try
                    {
                        string[] splits = keys[i].Split('_');
                        int indexX = int.Parse(splits[1]);
                        int indexY = int.Parse(splits[2]);
                        int lod = int.Parse(splits[3]);
        ...
        public Vector4 GetDecodedHeightMapSample(Vector2 uv)
        {
            ...
            float decodedHeight = GCommon.DecodeFloatRG(encodedHeight);
            c.x = decodedHeight;
            c.y = decodedHeight;
            return c;
        }
